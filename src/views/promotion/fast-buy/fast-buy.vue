    ...
    <c-layout>
        <c-content class="content">
            ...
            <div class="header" pa24>
                ...
                >
                    <c-columns pb20>
                        <span class="header__today" mr14 font-tertiary>今日必抢</span>
                        ...
                        <c-col span="8" v-for="(item, index) in hotGoodList" :key="index">
                            <c-box-skin radius4>
                                <c-magazine class="good__image" :image="item['image']">
                                    <c-button class="good__number" pv4 ph10 bga5 font-tertiary>{{
                                        item['sell']
                                    }}</c-button>
                                </c-magazine>
                                <div pa8 ellipsis1>{{ item['des'] }}</div>
                                <c-columns pb20>
                                    <div>
                                        <div style="color: #ed2c48" pl4>
                                            <b>{{ item['price'] }}</b>
                                        </div>
                                        <div
                                    ...
                                    <c-button class="good__buy" font-tertiary circle>抢</c-button>
                                </c-columns>
                            </c-box-skin>
                        </c-col>
                    </c-row>
            ...
            >
                <c-columns class="time">
                    <c-button class="time__container" v-for="(item, i) in time" :key="i">
                        <b class="time__t" :class="{ 'time__t--active': item['active'] }">{{
                            item['t']
                        }}</b>
                        <c-button
                            class="time__status"
            ...
            <c-button class="count">
                ...
                <c-button class="count__time" bga10 radius2>{{ hour }}</c-button>
                <span ph8>:</span>
                <c-button class="count__time" bga10 radius2>{{ min }}</c-button>
                <span ph8>:</span>
                <c-button class="count__time" bga10 radius2>{{ second }}</c-button>
                <span pl8>——</span>
            </c-button>
        ...
        <c-footer>
            <c-foot-nav class="footer" :list="footerList"></c-foot-nav>
        </c-footer>
    </c-layout>
</template>
<script>
...
export default {
    ...
    setup() {
        ...
        const timeDown = () => {
            ...
            const h = formate(parseInt((leftTime / (60 * 60)) % 24, 10))
            const m = formate(parseInt((leftTime / 60) % 60, 10))
            const s = formate(parseInt(leftTime % 60, 10))
            data.hour = h
            data.min = m
            data.second = s
        }
